...
import { VenueTimeSeriesPoint } from '@/hooks/useVenueTickets';
import { formatCurrency } from '@/utils/formatters';
import { parseEventName } from '@/utils/eventFormatters';
import { Calendar, TrendingUp, Star, Activity, MapPin, Building2 } from 'lucide-react';

interface VenueEventPortfolioAnalyticsProps {
...
export const VenueEventPortfolioAnalytics: React.FC<VenueEventPortfolioAnalyticsProps> = ({
  timeseries,
  eventsPresent = {},
  isLoading = false
}) => {
  const eventMetrics = useMemo(() => {
    ...
    // Calculate metrics for each event hosted at this venue
    const eventStats: { [eventId: string]: EventMetrics } = {};
    const totalVenueRevenue = timeseries.reduce((sum, day) => sum + day.daily_revenue, 0);
    ...
    timeseries.forEach(day => {
      day.events.forEach(eventId => {
        ...
        // For simplicity, we'll distribute the day's revenue/tickets equally among events at the venue
    ...
    Object.values(eventStats).forEach(event => {
      ...
      event.revenueShare = totalVenueRevenue > 0 ? (event.totalRevenue / totalVenueRevenue) * 100 : 0;
    });
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-semibold">Venue Event Portfolio</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  const getPerformanceBadge = (revenueShare: number) => {
    if (revenueShare >= 25) return { variant: 'default' as const, label: 'Top Event' };
    if (revenueShare >= 15) return { variant: 'secondary' as const, label: 'High Performer' };
    if (revenueShare >= 8) return { variant: 'outline' as const, label: 'Good Performer' };
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-sm font-semibold">
          ...
          Venue Event Portfolio Performance
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          Revenue contribution from {eventMetrics.length} events hosted at this venue
        </p>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
              ...
              <div key={event.eventId} className="space-y-2 p-3 rounded-lg border border-border/50 hover:bg-muted/30 transition-colors">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {event.revenueShare.toFixed(1)}% of venue total
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-muted-foreground">Days at Venue</div>
                  </div>
                  <div className="text-center">
                ...
                <div className="flex items-center justify-between pt-1">
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span>Hosted {event.activeDays} days</span>
                    </div>
                  </div>
